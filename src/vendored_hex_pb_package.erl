...
-module(vendored_hex_pb_package).
...
-spec encode_msg(map(),'RetirementStatus' | 'Dependency' | 'Release' | 'Package') -> binary().
encode_msg(Msg, MsgName) ->
    encode_msg(Msg, MsgName, []).


-spec encode_msg(map(),'RetirementStatus' | 'Dependency' | 'Release' | 'Package', list()) -> binary().
...
decode_msg(Bin, MsgName, Opts) when is_binary(Bin) ->
    ...
    case MsgName of
      'RetirementStatus' ->
	  try d_msg_RetirementStatus(Bin, TrUserData) catch
	    Class:Reason ->
		StackTrace = erlang:get_stacktrace(),
		error({gpb_error,
		       {decoding_failure,
			{Bin, 'RetirementStatus',
			 {Class, Reason, StackTrace}}}})
	  end;
      'Dependency' ->
	  try d_msg_Dependency(Bin, TrUserData) catch
	    Class:Reason ->
		StackTrace = erlang:get_stacktrace(),
		error({gpb_error,
		       {decoding_failure,
			{Bin, 'Dependency', {Class, Reason, StackTrace}}}})
	  end;
      'Release' ->
	  try d_msg_Release(Bin, TrUserData) catch
	    Class:Reason ->
		StackTrace = erlang:get_stacktrace(),
		error({gpb_error,
		       {decoding_failure,
			{Bin, 'Release', {Class, Reason, StackTrace}}}})
	  end;
      'Package' ->
    ...
    case Key of
	  ...
	  case Key band 7 of
		...
		skip_length_delimited_RetirementStatus(Rest, 0, 0, F1,
						       F2, TrUserData);
	    3 ->
		skip_group_RetirementStatus(Rest, Key bsr 3, 0, F1, F2,
					    TrUserData);
	    5 ->
...
skip_group_RetirementStatus(Bin, FNum, Z2, F1, F2,
			    TrUserData) ->
    {_, Rest} = read_group(Bin, FNum),
    dfp_read_field_def_RetirementStatus(Rest, 0, Z2, F1, F2,
					TrUserData).
    ...
    case Key of
	  ...
	  case Key band 7 of
		...
		skip_length_delimited_Dependency(Rest, 0, 0, F1, F2, F3,
						 F4, F5, TrUserData);
	    3 ->
		skip_group_Dependency(Rest, Key bsr 3, 0, F1, F2, F3,
				      F4, F5, TrUserData);
	    5 ->
...
skip_group_Dependency(Bin, FNum, Z2, F1, F2, F3, F4, F5,
		      TrUserData) ->
    {_, Rest} = read_group(Bin, FNum),
    dfp_read_field_def_Dependency(Rest, 0, Z2, F1, F2, F3,
				  F4, F5, TrUserData).
...
dfp_read_field_def_Release(<<>>, 0, 0, F1, F2, F3, F4,
    ...
    S2 = if F3 == '$undef' -> S1;
	    true ->
		S1#{dependencies => lists_reverse(F3, TrUserData)}
	 end,
    if F4 == '$undef' -> S2;
       true -> S2#{retired => F4}
    end;
    ...
    case Key of
	  ...
	  case Key band 7 of
		...
		skip_length_delimited_Release(Rest, 0, 0, F1, F2, F3,
					      F4, TrUserData);
	    3 ->
		skip_group_Release(Rest, Key bsr 3, 0, F1, F2, F3, F4,
				   TrUserData);
	    5 ->
...
skip_group_Release(Bin, FNum, Z2, F1, F2, F3, F4,
		   TrUserData) ->
    {_, Rest} = read_group(Bin, FNum),
    dfp_read_field_def_Release(Rest, 0, Z2, F1, F2, F3, F4,
			       TrUserData).
...
dfp_read_field_def_Package(<<>>, 0, 0, F1,
    ...
    S1 = #{},
    if F1 == '$undef' -> S1;
       true -> S1#{releases => lists_reverse(F1, TrUserData)}
    end;
    ...
    case Key of
	  ...
	  case Key band 7 of
		...
		skip_length_delimited_Package(Rest, 0, 0, F1,
					      TrUserData);
	    3 ->
		skip_group_Package(Rest, Key bsr 3, 0, F1, TrUserData);
...
skip_group_Package(Bin, FNum, Z2, F1, TrUserData) ->
    {_, Rest} = read_group(Bin, FNum),
    dfp_read_field_def_Package(Rest, 0, Z2, F1, TrUserData).
